...
→ 공통 사업 배경 이해
→ 제품별 판단과 실행 확인
→ 여러 제품을 동시에 관리한 방식 확인
→ 전체 성과와 배운 점
```

`Product Cases`를 `How I Worked`보다 먼저 배치한다. 제품별 Case 안에서 이미 `Challenge → Decision → Action → Result`를 보여주기 때문에, `How I Worked`는 제품별 실행을 반복하지 않고 **여러 제품을 동시에 운영한 공통 관리 방식**만 보여준다.
...
- `What I Worked On`의 각 제품 카드 내 **제품 사례 바로 보기 ↓** 클릭 → 같은 상세 페이지의 `#product-cases`로 smooth scroll
- 제품별 CTA를 클릭한 경우 `Product Cases` 도착 시 해당 제품 tab을 active 상태로 자동 선택
  - 선크림 카드 → 선크림 tab active
  - 다래 핸드크림 카드 → 핸드크림 tab active
  - 감자 바디로션 카드 → 바디로션 tab active
...
- `Product Cases`를 `How I Worked`보다 먼저 배치한다.
- 사용자가 제품 이미지를 본 직후 실제 사례를 바로 확인할 수 있게 한다.
- `Project Context`는 긴 설명이 아니라 **세 제품을 묶는 공통 사업 배경**만 2–3줄로 제공한다.
- `How I Worked`는 각 Case의 Action과 중복되지 않게 **다중 제품 일정/마일스톤 관리 방식**만 보여준다.
- active section만 accent color로 표시한다.
- 모바일에서는 simple anchor list 또는 accordion index를 사용한다.
...
각 제품의 `제품 사례 바로 보기 ↓`는 다른 route로 이동하지 않는다.
...
```text
선크림 CTA → Product Cases → 선크림 active
핸드크림 CTA → Product Cases → 핸드크림 active
바디로션 CTA → Product Cases → 바디로션 active
```

이 구조는 사용자가 `Project Context`를 먼저 읽지 않아도 실제 사례로 바로 이동할 수 있게 한다. 반대로 일반 스크롤을 계속하면 `Project Context → Product Cases` 순서로 자연스럽게 읽힌다.

---

# 16. Project Context

기존 `Why` 섹션을 `Project Context`로 변경한다.

이 섹션의 목적은 제품별 문제를 다시 설명하는 것이 아니라, **세 제품을 하나의 Featured Project로 묶는 공통 사업 배경**을 짧게 제시하는 것이다.
...
- 이미지 없음
- 별도 대형 비주얼 금지
- Product Cases로 넘어가기 전 맥락만 제공
- 제품별 `Challenge`와 내용이 반복되지 않도록 한다.
...
3개 제품의 상세 내용을 동시에 세로로 펼치지 않는다.

### Product Case UX

```text
What I Worked On 제품 카드
        ↓
제품 사례 바로 보기
        ↓
Product Cases로 smooth scroll
        ↓
선택한 제품 tab active
        ↓
...
각 Case는 “무엇을 했다”를 길게 나열하기보다 **판단이 어떻게 실행과 결과로 이어졌는지**가 빠르게 읽혀야 한다.
...
> 기존 제품군의 시리즈 확장 및 세트 구성을 위해 시작된 프로젝트입니다. 신규 제품군을 직접 경영진에 제안했으며, 승인 후 제품 개발부터 패키지·세트 구성·출시·프로모션까지 진행했습니다.
...
> 초기 출시 제품으로 최근 제품군과 패키지 디자인의 일관성이 떨어졌고, 제품 자체의 차별화 포인트도 충분히 드러나지 않는 상태였습니다.
...
이 섹션은 Product Cases 뒤에 배치한다.

각 제품 Case에 이미 `Action`이 있으므로, 여기서는 제품 개발 과정을 다시 설명하지 않는다. **여러 제품을 동시에 어떤 방식으로 관리했는지**만 보여준다.
...
실제 간트차트 자료를 사용할 수 있으면 민감정보를 제거해 재구성하고, 없다면 CSS로 심플하게 표현한다.
...
```text
제품별 Due Date가 서로 달랐다
→ 주요 마일스톤을 한 화면에 정리했다
→ 시험·패키지·프로모션 선후관계를 확인했다
→ 동시에 진행되는 제품의 누락을 줄였다
```

목적은 **프로젝트 관리 방식**을 보여주는 것. 장식용 차트로 만들지 않는다.
...
- What I Worked On의 제품 CTA가 Product Cases의 해당 tab으로 바로 연결되는가?
- Project Context가 제품별 Challenge를 반복하지 않고 공통 배경만 짧게 설명하는가?
- Detail Page에서 Challenge → Decision → Action → Result가 빠르게 스캔되는가?
- How I Worked가 제품별 Action을 반복하지 않고 다중 제품 관리 방식만 보여주는가?
...
> 여러 제품의 개발·패키지·시험·프로모션 일정이 동시에 진행되어, 제품별 주요 마일스톤과 마감일을 하나의 간트차트에서 함께 관리했습니다.

## Gantt Chart Direction

기존처럼 단계별 행만 두는 단순 차트보다, **하나의 간트차트 안에서 여러 제품 일정을 동시에 관리하는 형식**으로 구현한다.

### 목적

- 여러 제품이 병렬로 진행되었다는 점을 한눈에 보여준다.
- “제품별 일정 관리 방식”이 드러나도록 한다.
- 실제 기간이 아닌 **개념도**로 표현한다.

### 구조

- **행(Row)** = 제품명
- **열(Column)** = 공통 시간축
- 한 제품의 일정은 하나의 행에서 시간축을 따라 표시
- 각 행 내부에서 단계별 막대(bar)를 나눠 표현한다.
...
- 과하게 채도 높은 색상은 피한다.
- 전체 UI는 미니멀해야 하므로 muted tone 사용
- 색상은 제품 구분용으로만 사용하고, 차트 배경은 중립색 유지
- 색상만으로 구분하지 말고 제품명 라벨도 항상 함께 표시
...
퍼플 선 크림     ■■■ 제품 개발 → ■■ 패키지 → ■■ 시험 → ■■ 프로모션

다래 핸드크림         ■■ 제품 개발 → ■■ 패키지 → ■ 시험 → ■■ 프로모션

감자 바디로션              ■ 패키지 리뉴얼 → ■ Dermatest → ■ 화해 프로모션 → ■ 라이브커머스
```

실제 UI에서는 각 막대(bar)를 제품 컬러로 표시한다.

### 구현 규칙

- 하나의 차트에 3개 제품을 모두 포함한다.
- 제품별 주요 단계가 겹쳐 보이도록 병렬 구조를 유지한다.
- 너무 정밀한 일정표처럼 보이지 않게 하고, “운영 방식 설명용 개념도”임을 표기한다.
- 차트 상단 또는 좌측 상단에 아래 문구를 함께 넣는다.

> 일정 관리 방식 · 개념도 (실제 기간 아님)

### 추가 마일스톤 표시

필요 시 점(dot) 또는 작은 marker로 다음 이벤트를 표시할 수 있다.
...
단, 정보량이 많아지면 오히려 복잡해지므로 최대 3~4개 정도만 선택한다.

### 디자인 원칙

- 차트도 전체 포트폴리오 톤과 동일하게 미니멀하게 유지
- grid line은 매우 옅게
- legend는 작게
- 아이콘 남용 금지
- bar에 라운드 과하게 주지 않음
...
목적은 프로젝트 관리 방식을 보여주는 것이며, 장식용 차트처럼 보이면 안 된다.
